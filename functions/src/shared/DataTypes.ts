export type SocketDataType = {
    name: string; // name is the unique identifier. there are no ids
    // because then we could have overlapping names
    style: string;
    description: string;
    type: string;
...
/**
 * Creates a base socket style with the specified color.
 * @param {string} color - The color to use for the socket style.
 * @return {string} The CSS style string for the socket.
 */
...
export const DATATYPE_UNKNOWN = {
  ...
  style: baseSocketStyle("#FF00FF"), // magenta because that's like a
  // missing asset in a video game
  description:
        "Represents an unknown object. This could be anything. " +
        "Introspect to find out.",
  type: "unknown",
};
...
const standardDataTypes: SocketDataType[] = [
  ...
  },
  {
    ...
    description:
            "Represents an image stored as a JIMP " +
            "(JS image processing library) object.",
    type: "JimpInstance",
  },
  {
 ...
 * It first checks a local list and then would typically fall back to an API
 * if not found.
 * @param {string} name The name of the SocketDataType to retrieve.
 * @return {Promise<SocketDataType | undefined>} The SocketDataType object if
...
export async function fetchSocketDataTypeByName(
  ...
  // // 2. If not found locally, ideally, you would check a database
  // // via an API here.
  ...
  //         return data;
  //     } else {
  //         console.error(`Error fetching data type '${name}':`,
  //                      response.statusText);
  //         return undefined;
  //     }
  // } catch (error) {
