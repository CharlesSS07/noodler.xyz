...
export const imageClassification = functions.https.onRequest(async (req, res) => {
  try {
    await authenticateRequest(req);
    
    const { inputs, parameters }: ImageClassificationRequest = req.body;
    const model = req.body.model || getDefaultModel('image-classification');
    
    if (!validateInput(res, !!inputs, 'Missing inputs field (base64 image or image URL)')) return;
...
export const automaticSpeechRecognition = functions.https.onRequest(async (req, res) => {
  try {
    await authenticateRequest(req);
    
    const { inputs, parameters }: AutomaticSpeechRecognitionRequest = req.body;
    const model = req.body.model || getDefaultModel('automatic-speech-recognition');
    
    if (!validateInput(res, !!inputs, 'Missing inputs field (base64 audio data)')) return;
...
export const tableQuestionAnswering = functions.https.onRequest(async (req, res) => {
  try {
    await authenticateRequest(req);
    
    const { inputs, parameters }: TableQuestionAnsweringRequest = req.body;
    const model = req.body.model || getDefaultModel('table-question-answering');
    
    if (!validateInput(res, !!(inputs && inputs.query && inputs.table), 'Missing inputs.query or inputs.table')) return;
...
export const textToImage = functions.https.onRequest(async (req, res) => {
  try {
    ...
    if (!validateInput(res, !!inputs, 'Missing inputs field (text prompt)')) return;
