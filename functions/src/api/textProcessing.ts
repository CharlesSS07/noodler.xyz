...
export const textClassification = functions.https.onRequest(async (req, res) => {
  try {
    await authenticateRequest(req);
    
    const { inputs, parameters }: TextClassificationRequest = req.body;
    const model = req.body.model || getDefaultModel('text-classification');
...
export const tokenClassification = functions.https.onRequest(async (req, res) => {
  try {
    await authenticateRequest(req);
    
    const { inputs, parameters }: TokenClassificationRequest = req.body;
    const model = req.body.model || getDefaultModel('token-classification');
...
export const questionAnswering = functions.https.onRequest(async (req, res) => {
  try {
    ...
    if (!validateInput(res, !!(inputs && inputs.question && inputs.context), 'Missing inputs.question or inputs.context')) return;
...
export const fillMask = functions.https.onRequest(async (req, res) => {
  try {
    ...
    if (!validateInput(res, !!(inputs && inputs.includes('[MASK]')), 'Missing inputs or [MASK] token not found')) return;
...
export const sentenceSimilarity = functions.https.onRequest(async (req, res) => {
  try {
    await authenticateRequest(req);
    
    const { inputs, parameters }: SentenceSimilarityRequest = req.body;
    const model = req.body.model || getDefaultModel('sentence-similarity');
    
    if (!validateInput(res, !!(inputs && inputs.source_sentence && inputs.sentences), 'Missing inputs.source_sentence or inputs.sentences')) return;
...
export const conversational = functions.https.onRequest(async (req, res) => {
  try {
    ...
    if (!validateInput(res, !!(inputs && inputs.text), 'Missing inputs.text field')) return;
