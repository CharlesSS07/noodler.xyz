...
import {FirestoreNodeBluePrintModel} from "./libs/FirestoreNodeBluePrint";
...
export async function updateNodeBlueprint(nid: string, nodeBlueprint: Partial<FirestoreNodeBluePrintModel>): Promise<void> {

    const nbp = await getNodeBluePrint(nid);

    if (nbp.is_frozen) {
        throw new Error(`This node is frozen: ${nid}. Cannot change a frozen node.`);
    }

    function existsAndNotEqual(newValue: any, oldValue: any): boolean {
        return newValue !== undefined && JSON.stringify(newValue) !== JSON.stringify(oldValue);
    }

    // Owner (note: ownership changes are not allowed based on the setter implementation)
    ...
    if (nodeBlueprint.official_note !== undefined && nodeBlueprint.official_note !== nbp.official_note) {
    ...
    // Input spec strict
    if (existsAndNotEqual(nodeBlueprint.input_spec_strict, nbp.input_spec_strict)) {
    ...
    if (existsAndNotEqual(nodeBlueprint.input_sockets, nbp.inputSockets)) {
        ...
        console.warn("Input socket updates should be handled via newInputSocket method");
    }

    if (existsAndNotEqual(nodeBlueprint.output_sockets, nbp.outputSockets)) {
        // Note: This is complex as it requires comparing socket objects
        // For now, we'll assume the caller handles socket updates separately
        console.warn("Output socket updates should be handled via newOutputSocket method");
    }
