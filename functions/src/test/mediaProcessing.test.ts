...
describe("Media Processing Endpoints", () => {
    ...
    it("should transcribe audio successfully", async () => {
      ...
      const req = createMockRequest({
        inputs:
          ...
          "UklGRjIAAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQ4AAAC...",
      });
      const res = createMockResponse();
  ...
  describe("textToImage", () => {
    it("should generate image successfully", async () => {
      ...
      expect(res.data.metadata.prompt).to.equal(
        "Astronaut riding a horse"
      );
      expect(res.data.metadata.parameters.num_inference_steps).to.equal(
        10
      );
    });
    ...
    it("should return error for missing inputs", async () => {
      ...
      expect(res.statusCode).to.equal(400);
      expect(res.data.error).to.equal(
        "Missing inputs field (text prompt)"
      );
    });
