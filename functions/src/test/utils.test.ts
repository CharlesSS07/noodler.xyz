...
describe('Utils', () => {
  ...
  describe('authenticateRequest', () => {
    it('should authenticate valid request', async () => {
      const mockVerifyIdToken = sandbox.stub(admin.auth(), 'verifyIdToken').resolves(mockDecodedToken);
    ...
    it('should throw error for invalid authorization header format', async () => {
      const req = createMockRequest({}, { authorization: 'Invalid token' });
  ...
  describe('getDefaultModel', () => {
    it('should return correct default model for known tasks', () => {
      expect(getDefaultModel('text-generation')).to.equal('microsoft/DialoGPT-medium');
      expect(getDefaultModel('text-classification')).to.equal('cardiffnlp/twitter-roberta-base-sentiment-latest');
      expect(getDefaultModel('image-classification')).to.equal('google/vit-base-patch16-224');
      expect(getDefaultModel('text-to-image')).to.equal('black-forest-labs/FLUX.1-dev');
    });
