...
describe("Working HuggingFace API Integration Tests", () => {
  // These tests use models that are confirmed to work with the
      ...
      const result = await wrapHFResponse(
        client.questionAnswering({
          ...
          inputs: {
            ...
            context:
              "France is a country in Europe. The capital of France is " +
              "Paris. It is known for the Eiffel Tower.",
          },
        })
      );
      ...
      const result = await wrapHFResponse(
        client.questionAnswering({
          ...
          inputs: {
            ...
            context:
              "TechCorp was founded in 1995 by two college friends. " +
              "The company started as a small software development firm " +
              "and has grown to become a major player in the tech industry.",
          },
        })
      );
  ...
  describe("Sentence Similarity", () => {
    it("should compute sentence similarity with real API", async function() {
      ...
      expect(result.data[0]).to.be.greaterThan(result.data[1]);
      // Should be more similar to first sentence
  ...
  describe("Zero-shot Classification", () => {
    it("should perform zero-shot classification with real API",
      async function() {
      // eslint-disable-next-line no-invalid-this
        this.timeout(30000);
  ...
  describe("Text Summarization", () => {
    it("should perform text summarization with real API", async function() {
      ...
      const longText = `
        ...
        whose company designed and built the tower. Constructed from 1887 to
        1889 as the entrance to the 1889 World's Fair, it was initially
        criticized by some of France's leading artists and intellectuals for
        its design, but it has become a global cultural icon of France and
        one of the most recognizable structures in the world. The tower is
        330 metres (1,083 ft) tall, about the same height as an 81-storey
      ...
      if (Array.isArray(result.data)) {
        ...
        expect(result.data[0].summary_text.length).to.be.lessThan(
          longText.length
        );
        ...
        expect(result.data.summary_text.length).to.be.lessThan(
          longText.length
        );
