...
describe('Text Processing Endpoints', () => {
  ...
  describe('textGeneration', () => {
    it('should generate text successfully', async () => {
      ...
      expect(res.data).to.deep.equal([{ generated_text: 'Generated text' }]);
    });
  ...
  describe('textClassification', () => {
    it('should classify text successfully', async () => {
      ...
      expect(res.statusCode).to.equal(200);
      expect(res.data).to.deep.equal([[{ label: 'POSITIVE', score: 0.999 }]]);
    });
  ...
  describe('tokenClassification', () => {
    it('should perform NER successfully', async () => {
      const mockHFResponse = { 
        data: [{ entity_group: 'PER', score: 0.999, word: 'John', start: 0, end: 4 }], 
        status: 200 
      };
      sandbox.stub(utils, 'wrapHFResponse').resolves(mockHFResponse);
      ...
      expect(res.statusCode).to.equal(200);
      expect(res.data).to.deep.equal([{ entity_group: 'PER', score: 0.999, word: 'John', start: 0, end: 4 }]);
    });
  });
  ...
  describe('fillMask', () => {
    it('should fill mask successfully', async () => {
      const mockHFResponse = { 
        data: [{ score: 0.123, token: 2204, token_str: 'nice', sequence: 'the weather is nice.' }], 
        status: 200 
      };
      sandbox.stub(utils, 'wrapHFResponse').resolves(mockHFResponse);
  ...
  describe('summarization', () => {
    it('should summarize text successfully', async () => {
      ...
      const req = createMockRequest({
        inputs: 'This is a very long text that needs to be summarized into a shorter version.'
      });
      const res = createMockResponse();
  ...
  describe('sentenceSimilarity', () => {
    it('should compute sentence similarity successfully', async () => {
      ...
      const req = createMockRequest({
        inputs: {
          ...
          sentences: ['That is a very happy person', 'That is a sad person', 'Today is sunny']
        }
      });
      const res = createMockResponse();
    ...
    it('should return error for missing source_sentence or sentences', async () => {
      const req = createMockRequest({
      ...
      expect(res.statusCode).to.equal(400);
      expect(res.data.error).to.equal('Missing inputs.source_sentence or inputs.sentences');
    });
  });
  ...
  describe('conversational', () => {
    it('should generate conversational response successfully', async () => {
      ...
      expect(res.statusCode).to.equal(200);
      expect(res.data.generated_text).to.equal('That sounds interesting!');
    });
