...
export class FirebaseRTDBSocketSpecifier
    implements SocketSpecifierInstanceInterface
{
    socket_key: SocketID;
    node_key: string;
    isInput: boolean;
    ...
    getSocketPath() {
        if (this.isInput)
            return `${this.node_key}/input_sockets/${this.socket_key}`;
...
export class InputSocketInstanceController
    implements InputSocketInstanceControllerInterface
{
    ...
    getSocketRef() {
        return child(
            child(this.node.getNodeRef(), 'input_sockets'),
            this.socket_key.toString()
        );
    }
    ...
    async setValue(value: unknown) {
        if (value instanceof DoNotUploadToCloud) {
            SocketDataCache.setSocketData(
                this.getSocketInstanceSpecifier(),
                value.value
            );
    ...
    async getLink(): Promise<FirebaseRTDBSocketSpecifier | null> {
        return await get(this.getSocketRef()).then((snapshot) => {
            if (snapshot.exists())
                return snapshot.child('linkFromSocket').val();
            return null;
        });
    }
    ...
    getSocketInstanceSpecifier() {
        return new FirebaseRTDBSocketSpecifier(
            this.node.node_key,
            this.socket_key,
            true
        );
...
export class FirebaseRTDBNodeInstanceController
    implements NodeInstanceControllerInterface
{
    project: FirebaseRTDBProjectController;
    readonly node_key: string;
    private inputSocketInstanceControllers: Map<
        SocketID,
        InputSocketInstanceController
    > = new Map<SocketID, InputSocketInstanceController>();
    ...
    async call(): Promise<void> {
        ...
        try {
            (await this.getInputs()).forEach(
                (
                    socketInstanceParameters: InputSocketInstanceParameters,
                    socket_key
                ) => {
                    if (socketInstanceParameters.linkFromSocket) {
                        ...
                        if (
                            socketInstanceParameters.value ===
                            DoNotUploadToCloud.LOCALY_CACHED
                        ) {
            ...
            const opConfigController = new FirestoreNodeBluePrintController(
                await this.getNid()
            );

            opConfigController
                .getOutputSocketKeysInOrder()
                .then((outputSocketKeys: string[]) => {
            ...
            const outputValueCollection = new OutputSocketDataCollection(
                new Set<SocketID>(
                    await opConfigController.getOutputSocketKeysInOrder()
                )
            );
            (await opConfigController.getOutputSocketKeysInOrder()).forEach(
                (socketKey: SocketID) => {
                    outputValueCollection.on(
                        socketKey,
                        async (value: unknown) => {
                            SocketDataCache.setSocketData(
                                new FirebaseRTDBSocketSpecifier(
    ...
    async dumpCaches(): Promise<void> {
        (
            await new FirestoreNodeBluePrintController(
                await this.getNid()
            ).getOutputSocketKeysInOrder()
    ...
    getInputSocketController(
        socket_key: SocketID
    ): InputSocketInstanceController {
        let inputSocketInstanceController =
            this.inputSocketInstanceControllers.get(socket_key);
        if (inputSocketInstanceController) {
            return inputSocketInstanceController;
        }

        inputSocketInstanceController = new InputSocketInstanceController(
            this,
            socket_key
        );
        this.inputSocketInstanceControllers.set(
            socket_key,
            inputSocketInstanceController
        );
        return inputSocketInstanceController;
    ...
    async getInputs(): Promise<Map<SocketID, InputSocketInstanceParameters>> {
        return await get(this.getInputSocketsRef()).then((snapshot) => {
            ...
            return new Map<SocketID, InputSocketInstanceParameters>(
                Object.entries(input_sockets).map(
                    (value: [SocketID, InputSocketInstanceParameters]) => {
    ...
    async setInputValue(socket_key: SocketID, value: SocketValue) {
        const nodeBluePrint = await new FirestoreNodeBluePrintController(
            await this.getNid()
        );
        if (
            (await nodeBluePrint.getInputSocketKeysInOrder()).indexOf(
                socket_key
            ) != -1
        )
    ...
    async linkInputSocket(
        socket_key: SocketID,
        to: FirebaseRTDBSocketSpecifier
    ) {
        const config = await new FirestoreNodeBluePrintController(
            await this.getNid()
        );
        if (
            (await config.getInputSocketKeysInOrder()).indexOf(socket_key) != -1
        )
            await this.getInputSocketController(socket_key).setLink(to);
        else
            throw new Error(
                `Node ${await config.getTitle()} has no input socket named ${socket_key}`
            );
    }
