...
export class SocketDataCache {
	...
	static getSocketData(socket: SocketSpecifierInstanceInterface) {
		...
		throw new Error(`${socket.getUniqueIdentifier()} is not cached, no value found`);
	}

	static setSocketData(socket: SocketSpecifierInstanceInterface, value: unknown) {
		console.log(`set socket ${socket.getUniqueIdentifier()} to ${typeof value}`);
...
export class SocketElementCache {
	...
	static get(socket: SocketSpecifierInstanceInterface) {
		const socketElement = this.socketElementLookup.get(socket.getUniqueIdentifier());
		if (socketElement) return socketElement;
		throw new Error(`${socket.getUniqueIdentifier()} is has no registered socket element`);
	}
