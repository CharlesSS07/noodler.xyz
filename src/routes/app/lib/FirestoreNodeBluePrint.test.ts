...
describe('FirestoreNodeBluePrint Test Suite', () => {
    ...
    describe('1. Factory Method Tests', () => {
        it(
            'should create official node blueprint',
            async () => {
        ...
        it(
            'should create new custom node blueprint',
            async () => {
                const hint = `Custom Node ${Date.now()}`;
                const node = await factory.initNewNodeBluePrint(
                    testUser.uid,
                    hint
                );
        ...
        it(
            'should create new node blueprint without hint',
            async () => {
        ...
        it(
            'should fork existing node blueprint',
            async () => {
                // First create a node to fork
                const originalNode = await factory.initNewNodeBluePrint(
                    testUser.uid,
                    'Original Node'
                );
                ...
                // Fork the node
                const forkedNode = await factory.forkNode(
                    originalNode.nid,
                    testUser.uid
                );
    ...
    describe('2. Socket Management Tests', () => {
        ...
        beforeAll(async () => {
            testNode = await factory.initNewNodeBluePrint(
                testUser.uid,
                'Socket Test Node'
            );
        ...
        it('should add input sockets and retrieve them correctly', async () => {
            ...
            expect(testNode.inputSockets[0].params.default_value).toBe(
                'default test value'
            );
        });
    ...
    describe('3. Last Updated Tracking Tests', () => {
        ...
        it('should update last_updated_at when title is changed', async () => {
            ...
            expect(testNode.last_updated_at.getTime()).toBeGreaterThan(
    ...
    describe('5. Store Update Reactivity Tests', () => {
        it('should update store reactively with updateNodeBluePrintStore', async () => {
            ...
            expect(latestValue.documentation).toBe(
                'Reactively updated documentation'
            );
    ...
    describe('6. FirestoreNodeBluePrint Stress Tests', () => {
        ...
        it(
            'should handle concurrent node creation',
            async () => {
    ...
    describe('7. Combined Store and Firestore Stress Tests', () => {
        it(
            'should handle reactive updates under stress',
            async () => {
                            ...
                            new Promise<void>((resolve) => {
                                updateNodeBluePrintStore(
                                    ...
                                    async (node) => {
                                        ...
                                        node.newOutputSocket(
                                            `output_${storeIndex}_${updateIndex}`,
                                            socket
                                        );
                                        resolve();
                                    }
    ...
    describe('8. Code Execution Tests', () => {
        ...
        it('should handle code with error correctly', async () => {
            ...
            await expect(testNode.call(inputs, outputReturner)).rejects.toThrow(
                'Test error from node code'
            );
        });
        ...
        it('should fail execution on frozen node', async () => {
            ...
            const frozenNode = await factory.initOfficialNodeBluePrint(
                `frozen_test_${Date.now()}`
            );
