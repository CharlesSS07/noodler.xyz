...
describe("Jimp Import Tests", () => {
  ...
  test("Should be able to use Jimp.read() method", async () => {
    ...
    const originalImage = new Jimp({ width: 50, height: 50, color: '#00ff00' });
  ...
  test("Should be able to apply greyscale transformation", async () => {
    ...
    const colorImage = new Jimp({ width: 100, height: 100, color: '#ff00ff' });
  ...
  test("Should be able to crop an image", async () => {
    ...
    const largeImage = new Jimp({ width: 200, height: 200, color: '#0000ff' });
    
    // Crop to smaller size - use object format for crop parameters
    const croppedImage = largeImage.clone().crop({ x: 50, y: 50, w: 100, h: 100 });
  ...
  test("Should be able to resize an image", async () => {
    ...
    // Create an image
    const originalImage = new Jimp({ width: 100, height: 100, color: '#ffff00' });
  ...
  test("Should be able to get base64 representation", async () => {
    ...
    const base64 = base64Result instanceof Promise ? await base64Result : base64Result;
  ...
  test("Should verify utils object structure for node execution", async () => {
    ...
    const testImage = new utils.Jimp({ width: 25, height: 25, color: '#123456' });
  ...
  test("Should be able to read from buffer like image loader", async () => {
    ...
    const originalImage = new Jimp({ width: 64, height: 64, color: '#789abc' });
